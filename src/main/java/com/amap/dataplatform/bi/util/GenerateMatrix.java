...
public class GenerateMatrix {
	...
	public GenerateMatrix(int R,int C)
	{
		this.R = R ;
		this.C = C ;
		data = new double[R][C];
	...
	public GenerateMatrix(double[][]data)
	{
		R = data.length;
		C = data[0].length;
    ...
    // return V = A + B
    public GenerateMatrix plus(GenerateMatrix B) {
    	GenerateMatrix A = this;
        if (B.R != A.R || B.C != A.C) throw new RuntimeException("Illegal matrix dimensions.");
        GenerateMatrix V = new GenerateMatrix( R , C);
        for (int i = 0; i < R; i++)
            for (int j = 0; j < C ; j++)
	...
	public static void main(String[] args) {
		...
		GenerateMatrix gm = new GenerateMatrix(a);
		gm.show();
