...
public class StatNaviBusMapper extends
	...
	public void map(Object key,Text value,Context context) throws IOException, InterruptedException
		{
			...
			String[] inSplit = value.toString().split(ConstantsParseInput.tableFieldsSeparator,
					ConstantsParseInput.servwsfieldlen_navbus);
			
			if(inSplit.length!= ConstantsParseInput.servwsfieldlen_navbus)
			{
				...
				String model = inSplit[56].trim().replaceAll("\\.", "");;
				String diu	 = inSplit[31].trim();	
				String diu2	 = inSplit[32].trim();
				String diu3	 = inSplit[33].trim();
