...
import com.amap.dataplatform.bi.southpointer.dataprep.map.StatGISSomeOneMapper;
...
public class StatGISSomeOneJob extends Configured implements Tool{
	public static void main(String[] args) throws Exception {
		int exitCode = ToolRunner.run(new StatGISSomeOneJob(),  args);
	...
	public int run(String[] arg0) throws Exception {
		...
		Job job = new Job(conf,"stat_top_user_gis job");
	 	job.setJarByClass(StatGISSomeOneJob.class);
    	job.setMapperClass(StatGISSomeOneMapper.class);
    	job.setMapOutputKeyClass(Text.class);
    	job.setMapOutputValueClass(NullWritable.class);
    	job.setOutputKeyClass(Text.class);
    	job.setOutputValueClass(NullWritable.class);
    //	job.setNumReduceTasks(100);
		String inputPathString = conf.get("mapred.job.statusergissomeone.input.datapath.template", "");
		...
		String outputPathString = conf.get("mapred.job.statusergissomeone.output.datapath.template", "");
