...
public class StatAosTrafficMapper extends
	...
	public void map(Object key,Text value,Context context) throws IOException, InterruptedException
		{
			...
			String[] inSplit = value.toString().split(ConstantsParseInput.tableFieldsSeparator,
					ConstantsParseInput.tableAosTrafficFieldsLength);
			
			if(inSplit.length!= ConstantsParseInput.tableAosTrafficFieldsLength)
			{
				...
				diu = inSplit[25].trim();		
				//20级像素坐标转换成经纬度坐标
				String xstr = inSplit[50].trim();
				String ystr = inSplit[51].trim();
				...
				try {
					...
					{
						x = Double.parseDouble(xstr)/1000000;
						y = Double.parseDouble(ystr)/1000000;
